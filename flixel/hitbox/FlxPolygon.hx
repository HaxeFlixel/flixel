...
class FlxPolygon implements IFlxHitbox {
	...
	public function new(Sprite : FlxSprite, Vertices : Array<FlxPoint>)
	{
		...
		transformedVertices = new Array<FlxVector>();
		
		vertices = new Array<FlxVector>();
		
		for (i in 0...Vertices.length)
		{	
			vertices[i] = FlxVector.get(Vertices[i].x, Vertices[i].y);
	...
	public function updateTransformed()
	{
		...
		transMatrix.rotate(-parent.angle);
		...
		for (i in 0...vertices.length)
		{
			...
			transMatrix.transformPoint(transformedVertices[i]);
		}
