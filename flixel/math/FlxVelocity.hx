...
{
	/**
	 * Sets the source `FlxSprite`'s `velocity` so it will move directly towards the destination `FlxSprite` at the speed given (in pixels per second).
	 * If you specify a `maxTime`, it will adjust the speed (overwriting what you set) so it arrives at the destination in that number of seconds.
	...
	/**
	 * Sets the `acceleration` on the source `FlxSprite` so it will move towards the destination `FlxSprite` at the speed given (in pixels per second).
	...
	#if FLX_MOUSE
	/**
	 * Moves the given `FlxSprite` towards the mouse pointer coordinates at a steady velocity.
	 * If you specify a `maxTime`, it will adjust the speed (overwriting what you set) so it arrives at the destination in that number of seconds.
	 * Timings are approximate due to the way Flash timers work, and irrespective of SWF frame rate. Allow for a variance of +- 50ms.
	 * The source object doesn't stop moving automatically should it ever reach the destination coordinates.
	 *
	...
	#if FLX_TOUCH
	/**
	 * Moves the given `FlxSprite` towards a `FlxTouch` point at a steady velocity.
	 * If you specify a `maxTime`, it will adjust the speed (overwriting what you set) so it arrives at the destination in that number of seconds.
	 * Timings are approximate due to the way Flash timers work, and irrespective of SWF frame rate. Allow for a variance of +- 50ms.
	 * The source object doesn't stop moving automatically should it ever reach the destination coordinates.
	 *
	...
	#if FLX_MOUSE
	/**
	 * Sets the `acceleration` on the source `FlxSprite` so it will move towards the mouse coordinates at the speed given (in pixels per second).
	 * You must give a maximum speed value, beyond which the `FlxSprite` won't go any faster.
	...
	#if FLX_TOUCH
	/**
	 * Sets the `acceleration` on the source `FlxSprite` so it will move towards a `FlxTouch` at the speed given (in pixels per second).
	 * You must give a maximum speed value, beyond which the `FlxSprite` won't go any faster.
	 * If you don't need `acceleration`, look at `moveTowardsMouse()` instead.
	 *
	 * @param Source The `FlxSprite` on which the acceleration will be set.
	 * @param Touch The `FlxTouch` towards which to accelerate.
	 * @param Acceleration The speed it will accelerate in pixels per second.
	 * @param MaxSpeed The maximum speed in pixels per second in which the sprite can move.
	 */
	...
	/**
	 * Sets the `velocity` on the source `FlxSprite` so it will move towards the target coordinates at the speed given (in pixels per second).
	 * If you specify a `maxTime`, it will adjust the speed (overwriting what you set) so it arrives at the destination in that number of seconds.
	 * Timings are approximate due to the way Flash timers work, and irrespective of SWF frame rate. Allow for a variance of +- 50ms.
	 * The source object doesn't stop moving automatically should it ever reach the destination coordinates.
	 *
	...
	/**
	 * Sets the `acceleration` on the source `FlxSprite` so it will move towards the target coordinates at the speed given (in pixels per second).
	 * You must give a maximum speed value, beyond which the `FlxSprite` won't go any faster.
	...
	/**
	 * Given the angle and speed, calculates the velocity and returns it as an `FlxPoint`.
	 *
	 * @param Angle The angle (in degrees) calculated in clockwise positive direction (down = 90 degrees positive, right = 0 degrees positive, up = 90 degrees negative).
	 * @param Speed The speed it will move, in pixels per second squared.
	...
	/**
	 * Given the `FlxSprite` and speed, calculates the velocity and returns it as an `FlxPoint` based on the direction the sprite is facing.
	 *
	...
	/**
	 * Sets the `acceleration` on the source `FlxSprite` so it will accelerate in the direction of the specified angle.
	 * You must give a maximum speed value (in pixels per second), beyond which the `FlxSprite` won't go any faster.
	 *
	 * @param source The `FlxSprite` on which the acceleration will be set.
	 * @param radians The angle in which the `FlxSprite` will be set to accelerate.
	 * @param acceleration The speed it will accelerate in pixels per second.
	 * @param maxSpeed The maximum speed in pixels per second in which the sprite can move.
	 * @param resetVelocity Whether to reset the `FlxSprite`'s `velocity` to `0` each time.
	 */
