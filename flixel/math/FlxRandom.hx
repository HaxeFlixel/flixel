...
class FlxRandom
{
	...
	/**
	 * Creates a new `FlxRandom` object.
	 *
	 ...
	 *
	 * @param MeanThe Mean around which the normal distribution is centered.
	 ...
	 * call `bool(30.5)`. `true` means the chance passed, and `false` means it failed.
	 *
	...
	/**
	 * Pseudorandomly selects from an array of weighted options. For example, if you passed in an array of `[50, 30, 20]`
	 * there would be a 50% chance of returning a 0, a 30% chance of returning a `1`, and a 20% chance of returning a `2`.
	 * Note that the values in the array do not have to add up to 100 or any other number.
	 ...
	 * @return A value between `0` and `(WeightsArray.length - 1)`, with a probability equivalent to the values in `WeightsArray`.
	 */
	 ...
	 * @param GreyScale Whether to create a color that is strictly a shade of grey. `false` by default.
	 ...
	 * integer overflow in the `generate()` function.
	 */
	...
	/**
	 * Updates the seed that was used to create the most recent state.
	...
	/**
	 * Used to store the seed for a requested recording. If `StandardMode == false`, this will also reset the global seed!
	 * This ensures that the state is created in the same way as just before the recording was requested.
	 *
	 * @param StandardMode Whether to reset the entire game rather than just the current state.
	 */
