 ...
 */
@:allow(flixel.math.FlxRandomInstance)
class FlxRandom
{
	/**
	 ...
	 */
	public static var globalSeed(get, set):Int;
	/**
	 ...
	 */
	public static inline function int(Min:Int = 0, Max:Int = FlxMath.MAX_VALUE_INT, ?Excludes:Array<Int>):Int
	{
		return instance.int(Min, Max, Excludes);
	}
	 ...
	 * @param   Excludes   Optional array of values that should not be returned.
	 */
	public static inline function float(Min:Float = 0, Max:Float = 1, ?Excludes:Array<Float>):Float
	{
		return instance.float(Min, Max, Excludes);
	}
	...
	{
		return instance.chanceRoll(Chance);
	}
	 ...
	 */
	public static inline function weightedPick(WeightsArray:Array<Float>):Int
	{
		return instance.weightedPick(WeightsArray);
	}
	 ...
	 */
	@:generic
	public static inline function getObject<T>(Objects:Array<T>, ?WeightsArray:Array<Float>, StartIndex:Int = 0, ?EndIndex:Null<Int>):T
	{
		return instance.getObject(Objects, WeightsArray, StartIndex, EndIndex);
	}
	 ...
	 */
	@:generic
	public static inline function shuffleArray<T>(Objects:Array<T>, HowManyTimes:Int):Array<T>
	{
		return instance.shuffleArray(Objects, HowManyTimes);
	}
	 ...
	 */
	public static inline function color(?Min:FlxColor, ?Max:FlxColor, ?Alpha:Null<Int>, GreyScale:Bool = false):FlxColor
	{
		return instance.color(Min, Max, Alpha, GreyScale);
	}
	
	/**
	 * Internal storage of a FlxRandom instance.
	 */
	private static var instance:FlxRandomInstance = new FlxRandomInstance();
	
	private static inline function set_globalSeed(Seed:Int):Int
	{
		return instance.seed = Seed;
	}
	
	private static inline function get_globalSeed():Int
	{
		return instance.seed;
	}
	
	private static inline function set_currentSeed(Seed:Int):Int
	{
		return instance.currentSeed = Seed;
	}
	...
	{
		return _stateSeed = instance.currentSeed;
	}
	...
	{
		return _recordingSeed = instance.seed = StandardMode ? globalSeed : _stateSeed;
	}
...
/**
 * Code below is for an instance of FlxRandom.
 */
class FlxRandomInstance
{
	/**
	 * The base random number generator seed (for deterministic behavior in recordings and saves).
	 * If you want, you can set the seed with an integer between 1 and 2,147,483,647 inclusive.
	 * Altering this yourself may break recording functionality!
	 */
	public var seed(default, set):Int = 1;
	/**
	 * Current seed used to generate new random numbers. You can retrieve this value if,
	 * for example, you want to store the seed that was used to randomly generate a level.
	 */
	...
	/**
	 * Creates an instance of FlxRandom, if you need consistent deterministic numbers
	 * 
	 * @param	InitialSeed
	 */
	 ...
	 * @return  The new global seed.
	 */
	public inline function resetSeed():Int
	{
		return seed = FlxRandom.rangeBound(Std.int(Math.random() * FlxMath.MAX_VALUE_INT));
	}
	...
	public function int(Min:Int = 0, Max:Int = FlxMath.MAX_VALUE_INT, ?Excludes:Array<Int>):Int
	{
		...
		else
		{
			...
			if (Excludes == null)
			{
				return Math.floor(Min + generate() / FlxRandom.MODULUS * (Max - Min + 1));
			}
			else
			{
				var result:Int = 0;
				...
				do
				{
					result = Math.floor(Min + generate() / FlxRandom.MODULUS * (Max - Min + 1));
				}
	...
	public function float(Min:Float = 0, Max:Float = 1, ?Excludes:Array<Float>):Float
	{
		...
		{
			return generate() / FlxRandom.MODULUS;
		}
		else if (Min == Max)
		{
			result = Min;
		...
		else
		{
			...
			if (Excludes == null)
			{
				result = Min + (generate() / FlxRandom.MODULUS) * (Max - Min);
			}
			else
			{
				do
	 ...
	 * @return  Whether the roll passed or not.
	 */
	public inline function chanceRoll(Chance:Float = 50):Bool
	{
	 ...
	 */
	private var _arrayFloatHelper:Array<Float> = null;
	
	/**
	 * Internal function to update the internal seed whenever the global seed is reset,
	 * and keep the global seed's value in range.
	 */
	private inline function set_seed(NewSeed:Int):Int
	{
		return seed = currentSeed = FlxRandom.rangeBound(NewSeed);
	}
	...
	{
		return Std.int(internalSeed = FlxRandom.rangeBound(NewSeed));
	}
	...
	{
		return internalSeed = (internalSeed * FlxRandom.MULTIPLIER) % FlxRandom.MODULUS;
	}
}