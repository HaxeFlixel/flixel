...
class FlxCollision 
{
	public inline static var CAMERA_WALL_OUTSIDE:Int = 0;
	public inline static var CAMERA_WALL_INSIDE:Int = 1;
	...
	public static function pixelPerfectCheck(Contact:FlxSprite, Target:FlxSprite, AlphaTolerance:Int = 255, ?Camera:FlxCamera):Bool
	{
		...
		if (considerRotation)
		{
			...
			var testA2:BitmapData = FlxBitmapDataPool.get(Math.floor(boundsA.width), Math.floor(boundsA.height), true, 0x00000000, false);
			...
			var testB2:BitmapData = FlxBitmapDataPool.get(Math.floor(boundsB.width), Math.floor(boundsB.height), true, 0x00000000, false);
