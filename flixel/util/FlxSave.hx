 ...
 * a `FlxGame` is created at `FlxG.save`. The default `name` and `path` are specified by your
 ...
 * 
 * ## 5.0.0 Migration
 * In older versions of Flixel, a `null` path on HTML5 would use the current URL. `FlxSave`s with a
 ...
 * 
 * Previously, on desktop targets, saves were added to subdirectories based on the project's name and
 * company (i.e., `"/<app company>/<app title>/<localPath>/<name>.sol"`). This prevents separate
 * projects from referencing each other's saves (this was OpenFL attempting to mirror Flash's
 * `SharedObject` behavior). To allow cross-save referencing, the new location is simply:
...
class FlxSave implements IFlxDestroyable
{
	...
	/**
	 * Whether the save was successfully bound.
	 * @since 5.0.0
	 */
	 ...
	 * Mainly used to differentiate from other `FlxSave`s. If you do not specify
	 * this parameter, the company name specified in your `Project.xml` is used.
	 * @return Whether you successfully connected to the save data.
	 */
	 ...
	 * @return Whether you successfully found, merged and flushed data.
	 */
	 ...
	 * @param overwrite Whether the data should overwrite, should the 2 saves share data fields. Defaults to `false`.
	 * @param minFileSize If you need X amount of space for your save, specify it here.
	 * @return Whether you successfully saved the data.
	 */
	 ...
	 *
	 * @param minFileSize If you need X amount of space for your save, specify it here.
	 * @return Whether the data was written immediately. `false` could be an error OR a storage request popup.
	 */
	...
	/**
	 * Handy utility function for checking and warning if the shared object is bound yet.
	 *
 ...
 * - Mac: ```"/Users/<username>/Library/Application Support/<localPath>/<name>.sol"```
 * 
 * If `localPath == null`, the `Project.xml`'s app company metadata is used. `FlxG.save`'s default bind
 * args are `bind(app.company, app.file)`.
 * 
 * ## Legacy Paths
 * OpenFL's default save location is created using app metadata from the `Project.xml`.
...
private class FlxSharedObject extends SharedObject
{
	...
	/**
	 * Checks whether the save exists. Checks both the old and new path.
	 */
	...
	/**
	 * Checks whether the save exists. Checks the NEW location.
	 */
	...
	/**
	 * Checks whether the save exists. Checks the LEGACY location.
	 */
