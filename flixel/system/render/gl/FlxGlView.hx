...
class FlxGlView extends FlxCameraView
{
	...
	public function new(camera:FlxCamera) 
	{
		...
		_scrollRect.addChild(renderView);
		#end
	...
	override public function destroy():Void 
	{
		...
		#if !flash
		FlxDestroyUtil.removeChild(_scrollRect, renderView);
		renderView = FlxDestroyUtil.destroy(renderView);
		#end
	...
	override public function updateInternals():Void 
	{
		...
		if (renderView != null)
		{
			renderView.x = -0.5 * camera.width * (camera.scaleX - camera.initialZoom) * FlxG.scaleMode.scale.x;
			renderView.y = -0.5 * camera.height * (camera.scaleY - camera.initialZoom) * FlxG.scaleMode.scale.y;
			
			renderView.scaleX = camera.totalScaleX;
			renderView.scaleY = camera.totalScaleY;
			
			#if FLX_DEBUG
			if (debugLayer != null)
			{
				debugLayer.x = renderView.x;
				debugLayer.y = renderView.y;
				
				debugLayer.scaleX = renderView.scaleX;
				debugLayer.scaleY = renderView.scaleY;
			}
			#end
		}
	#end
