...
class SoundFrontEnd
{
	 ...
	 * Function should take the form `myVolumeHandler(volume:Float):Void`.
	 */
	public var volumeHandler:Float->Void;

	#if FLX_KEYBOARD
	/**
	 * The keys used to increase volume (see `FlxG.keys` for the keys available).
	 * Default keys: + (and numpad +). Set to `null` to deactivate.
	 */
	public var volumeUpKeys:Array<FlxKey> = [PLUS, NUMPADPLUS];

	/**
	 * The keys used to decrease volume (see `FlxG.keys` for the keys available).
	 * Default keys: - (and numpad -). Set to `null` to deactivate.
	 */
	...
	/**
	 * Whether the `soundTray` should be shown when any of the
	 * `volumeUp`-, `volumeDown`-, or `muteKeys` are pressed.
	 */
	...
	/**
	 * The group to which sounds played via `playMusic()` are added unless specified otherwise.
	 */
	public var defaultMusicGroup:FlxSoundGroup = new FlxSoundGroup();

	/**
	 * The group to which sounds in `load()` / `play()` / `stream()` are added unless specified otherwise.
	 */
	...
	/**
	 * Sets up and plays a looping background soundtrack.
	 *
	 ...
	 * Leave this value set as `false` if you want to reuse this `FlxSound` instance.
	 * @param autoPlay Whether to play the sound.
	 * @param url A URL from which to load a sound. Only used if `embeddedSound = null`.
	 * @param onComplete Called when the sound finishes playing.
	 * @param onLoad Called when the sound finishes loading. Called immediately for successfully loaded embedded sounds.
	 ...
	 *
	 * @param embeddedSound Name of sound assets specified in your `Project.xml` file.
	 ...
	 * @param group The group to add this sound to.
	 * @param autoDestroy Whether to destroy this sound when it finishes playing.
	 * Leave this value set to `false` if you want to reuse this `FlxSound` instance.
	 * @param onComplete Called when the sound finishes playing.
	 * @return A `FlxSound` object.
	 */
	 ...
	 *
	 * @param url A URL from which to load a sound.
	 * @param volume How loud to play it (`0` to `1`).
	 * @param looped Whether to loop this sound.
	 * @param group The group to add this sound to.
	 * @param autoDestroy Whether to destroy this sound when it finishes playing.
	...
	/**
	 * Resumes playing existing sounds.
	 */
	 ...
	 *
	 * @param forceDestroy Whether to kill sounds even if `persist == true`.
	 */
	...
	/**
	 * Toggles whether the game is muted. Also shows the sound tray.
	 */
	...
	/**
	 * Changes the volume by a certain amount. Also shows the sound tray.
	 */
