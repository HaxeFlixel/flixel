 ...
 * with a bunch of graphics options and abilities.
 */
class FlxImage extends FlxObject
{
	...
	public function clone():FlxImage
	{
		return (new FlxImage()).loadGraphicFromImage(this);
	}
	 ...
	 */
	public function loadGraphicFromImage(Image:FlxImage):FlxImage
	{
		frames = Image.frames;
	 ...
	 * @return	This FlxImage instance (nice for chaining stuff together, if you're into that).
	 */
	public function loadAsset(Graphic:FlxGraphicAsset, Unique:Bool = false, ?Key:String):FlxImage
	{
	 ...
	 * @return	This FlxSprite instance (nice for chaining stuff together, if you're into that).
	 */
	public function makeGraphic(Width:Int, Height:Int, Color:FlxColor = FlxColor.WHITE, Unique:Bool = false, ?Key:String):FlxImage
	{
	...
	override public function draw():Void
	{
		if (frame == null)
		{
			loadAsset(FlxGraphic.fromClass(GraphicDefault));
		}
	...
	/**
	 * Stamps / draws another FlxImage onto this FlxImage. 
	...
	private function set_clipRect(rect:FlxRect):FlxRect
	{
		if (frames != null)
		{
			var frameIndex:Int = frames.getFrameIndex(frame);
