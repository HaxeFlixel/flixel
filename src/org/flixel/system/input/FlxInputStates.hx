...
class FlxInputStates implements Dynamic
{
    ...
    /**
    * Get an Array of MapObjects that are in a pressed state
    * @return	Array<MapObject> of keys that are currently pressed.
    */
    public function getIsDown():Array<MapObject>
    {
        var keysdown:Array<MapObject> = new Array<MapObject>();
        var i:Int = 0;
        while(i < _total)
        {
            var o:MapObject = _map[i++];
