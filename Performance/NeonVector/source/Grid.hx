...
class Grid
{
	...
	public function new(GridRectangle:Rectangle, NumColumns:Int, NumRows:Int, MajorGridSize:Int = 4)
	{
		...
		for (_y in 0...numRows + 1)
		{
			for (_x in 0...numColumns + 1)
			{
				...
				if (_x == 0)
					lineCommands[_index] = GraphicsPathCommand.MOVE_TO;
				else
					lineCommands[_index] = GraphicsPathCommand.LINE_TO;
		...
		for (_x in 0...numColumns + 1)
		{
			for (_y in 0...numRows + 1)
			{
				...
				if (_y == 0)
					lineCommands[_index + _n] = GraphicsPathCommand.MOVE_TO;
				else
					lineCommands[_index + _n] = GraphicsPathCommand.LINE_TO;
	...
	public function applyDirectedForce(Position:FlxPoint, Force:FlxPoint, Radius:Float):Void
	{
		...
		for (_mass in points)
		{
			...
			if (_distance < Radius)
				_mass.applyForce(10 * Force.x / (10 + _distance), 10 * Force.y / (10 + _distance));
		}
	}
	...
	public function applyImplosiveForce(Position:FlxPoint, Force:Float, Radius:Float):Void
	{
		...
		for (_mass in points)
		{
			...
			if (_distance < Radius)
			{
				_mass.applyForce(10 * Force * (Position.x - _mass.position.x) / (100 + _distance * _distance),
	...
	public function applyExplosiveForce(Position:FlxPoint, Force:Float, Radius:Float):Void
	{
		...
		for (_mass in points)
		{
			...
			if (_distance < Radius)
			{
				_mass.applyForce(100 * Force * (_mass.position.x - Position.x) / (10000 + _distance * _distance),
	...
	{
		_pt.x = 0.5 * ((2 * Pt1.x)
			+ t * ((-Pt0.x + Pt2.x) + t * ((2 * Pt0.x - 5 * Pt1.x + 4 * Pt2.x - Pt3.x) + t * (-Pt0.x + 3 * Pt1.x - 3 * Pt2.x + Pt3.x))));
		_pt.y = 0.5 * ((2 * Pt1.y)
			+ t * ((-Pt0.y + Pt2.y) + t * ((2 * Pt0.y - 5 * Pt1.y + 4 * Pt2.y - Pt3.y) + t * (-Pt0.y + 3 * Pt1.y - 3 * Pt2.y + Pt3.y))));
		return _pt;
	}
	#end
}
