...
class PlayState extends FlxState
{
	...
	{			
		...
		destination.y += 112;
		path = new FlxPath().start(sprite, [sprite.getMidpoint(), destination], 40, FlxPath.YOYO);
		add(sprite);
		...
		destination = sprite.getMidpoint();
		destination.x += 56;
		var path = new FlxPath().start(sprite, [sprite.getMidpoint(), destination], 40, FlxPath.YOYO);
		add(sprite);
